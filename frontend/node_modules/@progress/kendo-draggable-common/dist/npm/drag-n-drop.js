...
})(DRAG_AND_DROP_DISPATCH_ACTION = exports.DRAG_AND_DROP_DISPATCH_ACTION || (exports.DRAG_AND_DROP_DISPATCH_ACTION = {}));
...
/** @hidden */
exports.normalizeEvent = function (event, state) { return (isTouchEvent(event)
    ? ({
...
var noop = function () { };
/** @hidden */
exports.dispatchDragAndDrop = function (state, action, callbacks) {
    ...
    var event = exports.normalizeEvent(action.event, state);
    switch (event.type) {
        ...
        case DRAG_AND_DROP_DISPATCH_ACTION.START: {
            var scrollableParent_1 = overrideScrollableParent || utils_1.getScrollableParent(action.payload.element);
        ...
        case DRAG_AND_DROP_DISPATCH_ACTION.SCROLL:
            ...
            var scrollableParent = overrideScrollableParent || utils_1.getScrollableParent(element);
        ...
        case DRAG_AND_DROP_DISPATCH_ACTION.MOVE: {
            if (state.pressed) {
                if (state.autoScroll && event.originalEvent.type !== 'scroll') {
                    if (element) {
                        var document_1 = utils_1.getDocument(element);
                        var scrollableParent_2 = overrideScrollableParent || utils_1.getScrollableParent(document_1.elementFromPoint(event.clientX, event.clientY));
                        var newVelocity = utils_1.autoScrollVelocity(event.pageX, event.pageY, utils_1.scrollableViewPort(scrollableParent_2, utils_1.getWindow(element)));
                        onVelocityChange({
                ...
                var dropElement_1 = algorithms_1.intersect(hint || element, state.drops.map(function (drop) { return drop && drop.element; }).filter(function (d) { return d !== (hint || element); }));
                var drop = state.drops.find(function (drop) { return drop.element === dropElement_1; });
                if (drop
                    && dropElement_1
                    && utils_1.isPointerInsideContainer(event.clientX, event.clientY, overrideScrollableParent || utils_1.getScrollableParent(dropElement_1))
